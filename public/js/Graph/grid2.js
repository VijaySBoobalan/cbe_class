...
// global variables required for slow plotting interval
var plotPath;
var plotPathNum;
var plotPointNum;
...
function drawGrid3(context,contextLeft,contextTop,gridDetails,opt_fontSize,opt_minorColor,opt_majorColor,opt_xAxisColor,opt_yAxisColor,opt_borderColor,opt_originColor,opt_backColor,opt_showGrid,opt_showScales,opt_showLabels) {
	...
	********************************/
	...
	var majorColor = gridDetails.majorColor || opt_majorColor || /*invertedBackColor[9] || */'#999';
	...
	if (showScales == true) {
		...
		if (showXScale == true) {
			...
			if (mode == "rad") {
				...
				while (roundToNearest(xAxisPoint,0.001) <= roundToNearest(left+width,0.001)) {
					if (xAxisPoint >= left) {
						var value = roundToNearest(major*xMajorStep,placeValue);
						var axisValue = [String(value)];
						var textWidth = context.measureText(String(axisValue)).width;
						context.fillStyle = backColor;
						context.fillRect(xAxisPoint - textWidth / 2, y0DisplayPos + xScaleOffset-1, textWidth, gridFontSize * 1.1);
...
function drawSmoothCurve(ctx,contextLeft,contextTop,gridDetails,pointsArray,tension,numOfPoints,forceUpwards) {
	var gridPoints = convertCoordsGridToCanvas(contextLeft,contextTop,gridDetails,pointsArray);
