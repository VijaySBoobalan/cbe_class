...
<script type="text/javascript" src="https://cdn.jsdelivr.net/jquery.validation/1.16.0/additional-methods.min.js"></script>
    ...
    <div class="card">
        ...
        <table class="table datatable-basic" id="mess_menu_table">
            ...
            <tbody>
                @if (isset($StudentScholarShips))    
                    @foreach ($StudentScholarShips as $key=>$StudentScholarShip)
                        <tr>
                            <th>{{ ++$key }}</th>
                            <th>{{ $StudentScholarShip->scholarship_type }}</th>
                            <th>{{ $StudentScholarShip->year }}</th>
                            <th>{{ $StudentScholarShip->semester }}</th>
                            @if ($StudentScholarShip->scholarship_type == 'acadamic')
                                <th>{{ $StudentScholarShip->getScholarshipAcadamic->acadamic_scholarship_name }}</th>
                                <th>{{ $StudentScholarShip->percentage }}</th>
                                <th>{{ $StudentScholarShip->getScholarshipAcadamic->acadamic_fees_concertion }}</th>
                            @else
                                <th>{{ $StudentScholarShip->getScholarshipAcadamic->zonal_particulars }}</th>
                                <th>-</th>
                                <th>{{ $StudentScholarShip->getScholarshipAcadamic->zonal_fees_concertion }}</th>
                            @endif
                        </tr>
                    @endforeach
                @endif
